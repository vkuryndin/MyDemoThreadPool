 ...
 *
 * <p>These tests verify that a core worker must stay alive even if it remains idle longer than
 * keepAliveTime.
 */
class CustomThreadPoolCoreWorkerIdleTimeoutTest {

  /**
   * Verifies that the single core worker does not stop after idle timeout when corePoolSize =
   * maxPoolSize = 1.
   *
   * <p>In this scenario: - the pool starts with one core worker - no tasks are submitted - we wait
   * longer than keepAliveTime - the core worker must still exist
   */
  @Test
  void shouldKeepCoreWorkerAliveAfterIdleTimeout() throws Exception {
    ...
    try {
       ...
       * implementation to accidentally stop the worker.
       */
      Thread.sleep(1000);
   ...
   *
   * <p>This configuration makes it easy to verify that the core worker is never stopped by idle
   * timeout.
   *
  ...
  private CustomThreadPool createSingleCoreWorkerPool(String poolName) {
    PoolConfig config = new PoolConfig(1, 1, 300, TimeUnit.MILLISECONDS, 1, 0);
