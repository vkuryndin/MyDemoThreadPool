 ...
 *
 * Each worker has its own task queue and executes tasks from that queue.
 * If the worker stays idle longer than keepAliveTime, it may stop
 * if the pool allows it.
 */
public class Worker implements Runnable {

    /**
     * A reference to the pool controller.
     * The worker uses it to check pool state and report termination.
     */
     ...
     *
     * We mark it as volatile so that other threads can read
     * the latest value safely.
     */
     ...
     *
     * It helps prevent a race when the worker is about to stop
     * while another thread is trying to put a new task into its queue.
     */
    ...
    public Worker(WorkerController controller,
                  int queueSize,
                  long keepAliveTime,
                  TimeUnit timeUnit) {
     ...
     *
     * The method is synchronized with the worker stop decision,
     * so a task cannot be added to a worker that is already stopping.
     *
     ...
     *
     * This method does not affect the task that may already be running.
     * It only clears tasks that have not started yet.
     *
     ...
     * Main worker loop.
     *
     * The worker waits for tasks using poll(timeout). This allows it
     * to wake up after keepAliveTime and decide whether it should stop.
     */
    @Override
    public void run() {
        try {
            while (true) {
                ...
                /**
                 * In graceful shutdown mode, the worker should finish
                 * only after its queue becomes empty.
                 */
                ...
                /**
                 * If no task was received during the idle timeout,
                 * ask the pool whether this worker may stop.
                 *
                 * The stop decision is synchronized with task acceptance
                 * to avoid losing tasks during worker shutdown.
                 */
                if (task == null) {
                    synchronized (queueLock) {
                ...
                /**
                 * If immediate shutdown started after we received the task,
                 * do not execute it.
                 */
                if (controller.isShutdownNow()) {
                    break;
            ...
            /**
             * The worker may be interrupted during shutdownNow()
     ...
     *
     * One failed task must not kill the whole worker.
     *
    ...
    private void executeTask(Runnable task) {
        ...
        try {
            ...
            System.out.println("[Worker] " + Thread.currentThread().getName()
                    + " task failed: " + task + ", reason: " + e.getMessage());
        } finally {
            busy = false;
        }
    }
